...
use Reasno\HyperfMysqlPartition\Exception\UnexpectedValueException;
use Reasno\HyperfMysqlPartition\Exception\UnsupportedPartitionException;
use Reasno\HyperfMysqlPartition\Model\Partition;

/**
 * Class PartitionHelper method.
 */
class Schema extends \Hyperf\Database\Schema\Schema
{
    ...
    public static function havePartitioning()
    {
        if (!self::$already_checked && version_compare(self::version(), 5.1, '>=')) {
            if (version_compare(self::version(), 8.0, '>=')) {
                ...
                $plugins = DB::connection('default')->getPdo()->query("SHOW PLUGINS")->fetchAll();
